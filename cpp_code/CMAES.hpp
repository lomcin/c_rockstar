...
class CMAES{
 ...
 public:
  ...
  CMAES(const Eigen::MatrixBase<Derived>& initialParameterSet, const Eigen::MatrixBase<Derived>& std):
  optimizationDone(false),sample_index(0), hsig(0.0), n_rolloutsEvaluated(0)
  {
    ...
    if (std.cols() == 1)
      ...
      throw std::runtime_error("CMAES constructor: initial standard deviation should either column vector or a row vector!");

    if (initial_theta.rows() != StandardDev.rows())
      throw std::runtime_error("CMAES constructor: the length of initial standard deviation and the length of the initial policy parameter vector should be the same!");
 ...
 public:
  ...
  void getNextTheta2Evaluate(Eigen::MatrixBase<Derived>& nexTheta2Evaluate){
    ...
    if (nexTheta2Evaluate.cols() == 1)
      ...
      throw std::runtime_error("CMAES getNextTheta2Evaluate: new policy container should either column vector or a row vector!");
...
//        {
//          arx(j,k) = std::max(arx(j,k), lowerbound(j));
//          arx(j,k) = std::min(arx(j,k), upperbound(j));
//        }


        readyToSampleAgain = false;

        /////enforcing constraints by projection
      }
      if(isItColumnVector)
        nexTheta2Evaluate.block(0,0,NParameter,1) = (arx.col(0).cwiseProduct(StandardDev) + initial_theta);
      else
        nexTheta2Evaluate.block(0,0,1, NParameter) = (arx.col(0).cwiseProduct(StandardDev) + initial_theta).transpose();

      sample_index = 1;
    }else if(lambda == sample_index)
    {
      throw std::runtime_error("CMAES getNextTheta2Evaluate: Please input all the cost before asking for more policy :) ");
    }
    else{
      if(isItColumnVector)
        nexTheta2Evaluate.block(0,0,NParameter,1) = (arx.col(sample_index).cwiseProduct(StandardDev) + initial_theta);
      else
        nexTheta2Evaluate.block(0,0,1, NParameter) = (arx.col(sample_index).cwiseProduct(StandardDev) + initial_theta).transpose();
  ...
  void setTheCostFromTheLastTheta(double cost) {
    ...
    if(cost_index == lambda){
      ...
      hsig = ps.squaredNorm()/(1.0-pow((1.0-cs),(2.0*counteval/lambda)))/NParameter < 2.0 + 4.0/(NParameter+1.0);
      pc = (1-cc) * pc + hsig * sqrt(cc*(2.0-cc)*mueff) *(xmean-xold) / sigma;

      for(int i=0;i<mu;i++)
        artmp.col(i)=(arx.col(i)-xold)/sigma;

      C = (1.0-c1-cmu) * C + c1 * (pc * pc.transpose()+ (1.0-hsig) * cc*(2.0-cc) * C)+ cmu * artmp * weights.asDiagonal() * artmp.transpose();
  ...
  void getBestEverSeenPolicy(Eigen::MatrixBase<Derived>& bestPolicy){
    ...
    if(bestPolicy.cols() == NParameter)
      ...
      throw std::runtime_error("CMAES getBestEverSeenPolicy: Please input a vector (either a row vector or a column vector)");
  }
  ...
  void getEstimatedOptimalPolicy(Eigen::MatrixBase<Derived>& bestPolicy){
    ...
    if(bestPolicy.rows() == NParameter)
      ...
      throw std::runtime_error("CMAES getEstimatedOptimalPolicy: Please input a vector (either a row vector or a column vector)");
  }
